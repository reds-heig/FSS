...
static struct map_desc versatile_io_desc[] __initdata __maybe_unused = {
	{
		...
		.virtual	=  IO_ADDRESS(VERSATILE_FSS_OFFSET),
		.pfn		= __phys_to_pfn(VERSATILE_FSS_OFFSET),
		.length		= SZ_4K * 9,
		.type		= MT_DEVICE
	},
...
/*
 * Interrupt line for FSS
 */
#define FSS_IRQ           IRQ_VICSOURCE27
...
/*
 * IRQ number and memory region address for the FSS device
 */
static struct resource fss_resources[] = {
    [0] = {
	.start	= (FSS_IRQ),
	.end	= (FSS_IRQ),
	.flags	= IORESOURCE_IRQ,
    ...
    [1] = {
	.start	= (VERSATILE_FSS_OFFSET),
	.end	= (VERSATILE_FSS_OFFSET),
	.flags	= IORESOURCE_MEM,
    }
};

static struct platform_device fss_device = {
	.name			= "fss",
	.id			= 0,
	.num_resources		= 2,
	.resource		= fss_resources,
};
...
void __init versatile_init(void)
{
	...
	platform_device_register(&fss_device);
