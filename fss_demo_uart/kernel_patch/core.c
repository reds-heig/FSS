...
static struct map_desc versatile_io_desc[] __initdata __maybe_unused = {
	{
		...
		.virtual	=  IO_ADDRESS(VERSATILE_FSS),
		.pfn		= __phys_to_pfn(VERSATILE_FSS),
		.length		= SZ_4K * 9,
		.type		= MT_DEVICE
	},
...
static struct plat_serial8250_port serial_platform_data[] = {
	{
		.membase	= (void *)IO_ADDRESS(VERSATILE_FSS),
		.mapbase	= VERSATILE_FSS,
		.irq		= FSS_IRQ,
		.uartclk	= 24000000,
		.regshift	= 2,
		.iotype		= UPIO_MEM,
...
{
	return platform_device_register(&fss_device);
}
